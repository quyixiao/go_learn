...
//读者也许会好奇，为什么返回数组数据的方法有8个，但却只有一个统一的ArrayLength()方法呢?答案是，这些方法主要是供
//<t>aload、<t>astore和arraylength指令使用的。<t>aload和<t>astore系 列指令各有8条，所以针对每种类型都提供一个方法，
// 返回相应的 数组数据。因为arraylength指令只有一条，所以ArrayLength()方法 需要自己判断数组类型。
